...
const TeacherRolesPage = ({ params }: Props) => {
  const router = useRouter();

  const [loading, setLoading] = useState(true);

  const [teacherRoles, setTeacherRoles] = useState<TeacherRole[]>([]);
  const [teacherRolesCount, setTeacherRolesCount] = useState(0);
  ...
  const getAllTeacherRoles = async () => {
    setLoading(true);
    const res = await AdminServices.getAllTeacherRoles({ ...pagination, courseId: params.courseId });
    ...
    if (!res.data.status) {
      ...
      setTeacherRolesCount(res.data.data.teacherRoleCount);
    }
    ...
    <div className="h-full flex flex-col gap-2 p-2">
      <div className=" flex w-full items-end justify-between">
        <div className="pt-5 pb-5 flex items-center gap-2">
          ...
          <div className="flex gap-1">
            ...
            <h1 className="text-2xl font-extrabold">Teacher Roles</h1>
          </div>
        </div>
        <div className="flex gap-2 items-end">
          ...
          <Form refetch={getAllTeacherRoles}>
            <Button>
      ...
      >
        <Table>
          ...
          <TableBody>
            {teacherRoles?.map((teacherRole, index) => (
              <TableRow className="cursor-pointer" key={teacherRole.id}>
                <TableCell className="font-medium">
                  {index + 1 + (pagination.currentPage - 1) * pagination.itemsPerPage}
                </TableCell>
                <TableCell>{teacherRole.name}</TableCell>
                <TableCell>
                  {teacherRole.status ? (
                ...
                <TableCell>
                  <div className="flex gap-2">
                    <Form
                      refetch={getAllTeacherRoles}
                      teacherRoleId={teacherRole.id}
                      name={teacherRole.name}
                      status={teacherRole.status}
                    >
                    ...
                    <DeleteTableAction
                      action={async () => {
                        await AdminServices.deleteTeacherRole(teacherRole.id);
                        await getAllTeacherRoles();
                        toast({
                          title: "Teacher Role Deleted",
                          description: `'Teacher Role ${teacherRole.name}' was permanently deleted from this course.`,
...
export default TeacherRolesPage;
