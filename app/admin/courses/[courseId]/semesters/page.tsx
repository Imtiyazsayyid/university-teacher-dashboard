...
const SemestersPage = ({ params }: Props) => {
  ...
  const getAllSemesters = async () => {
    setLoading(true);
    const res = await AdminServices.getAllSemesters({ ...pagination, courseId: params.courseId });
    ...
    <div className="h-full flex flex-col gap-2 p-2">
      <div className=" flex w-full items-end justify-between">
        ...
        <div className="flex gap-2 items-end">
          ...
          <Form courseName={course?.name || ""} courseId={parseInt(params.courseId)} refetch={getAllSemesters}>
            <Button>
          ...
          <TableBody>
            {semesters?.map((semester, index) => (
              <TableRow
                ...
                key={semester.id}
                onClick={(e) => {
                  router.push(`/admin/courses/${params.courseId}/semesters/${semester.id}/subjects`);
                }}
              >
                ...
                <TableCell>
                  <div className="flex gap-2">
                    ...
                    <DeleteTableAction
                      action={async () => {
                        await AdminServices.deleteSemester(semester.id);
