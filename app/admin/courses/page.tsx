...
const CoursesPage = () => {
  ...
  const getAllCourses = async () => {
    ...
    const res = await AdminServices.getAllCourses({ ...filters, ...pagination });
          ...
          <TableBody>
            {courses?.map((course, index) => (
              <TableRow
                ...
                key={course.id}
                onClick={() => router.push(`/admin/courses/${course.id}/semesters`)}
              >
                ...
                <TableCell>
                  <div className="flex gap-2">
                    <EditTableAction action={() => router.push("/admin/courses/" + course.id)} />
                    <DeleteTableAction
                      action={async () => {
                        await AdminServices.deleteCourse(course.id);
                        await getAllCourses();
                        toast({
