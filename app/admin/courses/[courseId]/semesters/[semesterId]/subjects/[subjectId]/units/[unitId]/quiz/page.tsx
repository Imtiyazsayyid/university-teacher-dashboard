...
const UnitQuizesPage = ({ params }: Props) => {
  ...
  const getAllUnitQuizes = async () => {
    setLoading(true);
    const res = await AdminServices.getAllUnitQuizes({ ...pagination, unitId: params.unitId, ...filters });
      ...
      <div className=" flex w-full items-end justify-between">
        ...
        <div className="flex gap-2 items-end">
          ...
          <Button
            onClick={() =>
              router.push(
                `/admin/courses/${params.courseId}/semesters/${params.semesterId}/subjects/${subject?.id}/units/${params.unitId}/quiz/new`
              )
            }
          >
                ...
                <TableCell>
                  <div className="flex gap-2">
                    ...
                    <EditTableAction
                      action={() =>
                        router.push(
                          `/admin/courses/${params.courseId}/semesters/${params.semesterId}/subjects/${subject?.id}/units/${params.unitId}/quiz/${unitQuiz.id}`
                        )
                      }
                    />
                    <DeleteTableAction
                      action={async () => {
                        await AdminServices.deleteUnitQuiz(unitQuiz.id);
